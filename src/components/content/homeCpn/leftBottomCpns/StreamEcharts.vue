<template>
  <div id="streamEcharts" style="width:100%;height:100%"></div>
</template>

<script>
import { getCaptureSelectPullIn, getCaptureSelectPassing } from '@/api/oilStation'
export default {
  props: ['orgIdEchar'],
  name: 'streamEcharts',
  data() {
  ...
  methods: {
    ...
    async render() {
      ...
      // 过往车辆
      const res2 = await getCaptureSelectPassing({ orgId: this.orgIdEchar })
      ...
      this.legendData = [
        ...
        {
          name: '进站率',
          num: this.stopTheVehicleSum == 0 || this.vehiclesSum == 0 ? '0%' : ((this.stopTheVehicleSum / this.vehiclesSum) * 100).toFixed(2) + '%'
        }
      ]
      ...
      this.pullIn.forEach((it, index) => {
        if (it === 0) {
          ...
          if (this.passing[index] === 0) {
            ...
            const num = ((this.pullIn[index] / this.passing[index]) * 100).toFixed(2) + '%'
            this.feedRate.push(num)
          }
        }
      })
      console.log(this.pullIn, this.passing, this.feedRate);
      await this.drawLine('streamEcharts')
    }
  },
  async mounted() {
    // 进站车辆
